...
async function openDocumentHistory(cliBinary: string, env: NodeJS.ProcessEnv): Promise<OpenHistoryResult> {
    return await evalObsidianJson<OpenHistoryResult>(
        ...
        [
            ...
            "await new Promise((resolve)=>setTimeout(resolve,300));",
            "await app.commands.executeCommandById('livesync-fsx:livesync-history');",
...
async function main(): Promise<void> {
    ...
    const screenshotDir =
        ...
        join(process.env.E2E_OBSIDIAN_DIAGNOSTICS_DIR ?? "/tmp/livesync-fsx-e2e", "document-history-nav");
