import { decodeSettingsFromSetupURI, DEFAULT_SETTINGS } from "../setup/livesync-commonlib.ts";
...
Deno.test("generates a current self-hosted Setup URI through the published Commonlib contract", async () => {
    ...
    const setupURI = stdout.match(/obsidian:\/\/setuplivesync\?settings=\S+/)?.[0];
    ...
    assert(effectiveSettings.isConfigured, "the CouchDB Setup URI left the imported device unconfigured");
    assert(
    ...
    assert(effectiveSettings.E2EEAlgorithm === "v2", "the Setup URI did not use the current E2EE algorithm");
    assert(
    ...
    assert(profiles.length === 1, "the Setup URI did not contain exactly one CouchDB remote profile");
    assert(decoded.activeConfigurationId === profiles[0].id, "the CouchDB remote profile was not selected");
    assert(profiles[0].uri.startsWith("sls+https://"), "the selected remote profile was not a CouchDB connection URI");
});
