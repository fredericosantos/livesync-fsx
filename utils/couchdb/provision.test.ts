...
Deno.test("configures CouchDB and delegates database-version initialisation", async () => {
    ...
    assert(requests[0].url.endsWith("/_cluster_setup"), "cluster setup was not first");
    assert(
        requests.some(
            (request) => request.url.endsWith("/_config/cors/origins") && request.body.includes("app://obsidian.md")
        ),
        "the Obsidian CORS origins were not configured"
    );
    assert(requests.at(-1)?.url === "https://couch.example.test/notes", "the requested database was not created last");
    assert(initialisations.length === 1, "database-version initialisation was not delegated once");
    assert(
...
Deno.test("leaves database creation to the client when no database is supplied", async () => {
    ...
    assert(!initialised, "database-version initialisation ran without a database");
});
