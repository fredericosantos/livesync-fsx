...
interface ProvisioningDependencies {
    ...
    initialiseDatabaseVersion: (databaseURL: string, username: string, password: string) => Promise<void>;
}
...
async function requestWithRetry(
    ...
    for (let attempt = 1; attempt <= retryCount; attempt++) {
        try {
            ...
            lastError = error;
            if (error instanceof Error && error.message.startsWith(`${label} failed with HTTP 4`)) {
                throw error;
            }
        }
        if (attempt < retryCount) await dependencies.sleep(retryDelayMs);
    }
    throw lastError instanceof Error ? lastError : new Error(`${label} failed after ${retryCount} attempts`);
}
...
export async function initialiseLiveSyncDatabaseVersion(
    ...
    try {
        const compatible = await checkRemoteVersion(database, async () => false);
        if (!compatible) {
            throw new Error("the remote database uses an incompatible LiveSync database version");
        }
    } finally {
...
export async function provisionCouchDB(
    ...
    const configure = async (
        ...
        body: string,
        method = "PUT",
        accept: (response: Response, body: string) => boolean = (response) => response.ok
    ) =>
    ...
    await configure(
        ...
        "POST",
        (response, body) =>
            response.ok || ((response.status === 400 || response.status === 409) && /already|finished/i.test(body))
    );

    const settings: Array<[string, string, string]> = [
        ["require authenticated HTTP users", "chttpd/require_valid_user", '"true"'],
        ["require authenticated HTTP users for authentication", "chttpd_auth/require_valid_user", '"true"'],
        ["set the HTTP authentication challenge", "httpd/WWW-Authenticate", '"Basic realm=\\"couchdb\\""'],
        ["enable HTTP CORS", "httpd/enable_cors", '"true"'],
        ["enable clustered HTTP CORS", "chttpd/enable_cors", '"true"'],
        ["set the maximum HTTP request size", "chttpd/max_http_request_size", '"4294967296"'],
        ["set the maximum document size", "couchdb/max_document_size", '"50000000"'],
    ...
    if (options.database?.trim()) {
        ...
        await dependencies.initialiseDatabaseVersion(databaseURL, username, password);
    }
}
