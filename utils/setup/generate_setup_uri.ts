...
export type SetupGeneratorEnvironment = Readonly<Record<string, string | undefined>>;
...
function requireValue(environment: SetupGeneratorEnvironment, name: string): string {
    const value = environment[name]?.trim();
    if (!value) throw new Error(`${name} is required`);
    return value;
}

function optionalBoolean(environment: SetupGeneratorEnvironment, name: string, fallback: boolean): boolean {
...
function applyEncryptedVaultSettings(settings: ObsidianLiveSyncSettings, environment: SetupGeneratorEnvironment): void {
...
function createCouchDBSettings(environment: SetupGeneratorEnvironment): ObsidianLiveSyncSettings {
...
function createObjectStorageSettings(environment: SetupGeneratorEnvironment): ObsidianLiveSyncSettings {
    const settings = createNewVaultSettings();
    Object.assign(settings, PREFERRED_JOURNAL_SYNC, {
        ...
        useCustomRequestHandler: optionalBoolean(environment, "use_custom_request_handler", false),
...
function createP2PSettings(environment: SetupGeneratorEnvironment): ObsidianLiveSyncSettings {
    const settings = createNewVaultSettings();
    Object.assign(settings, PREFERRED_BASE, P2P_DEFAULT_SETTINGS, {
        ...
        P2P_relays: environment.p2p_relays?.trim() || P2P_DEFAULT_SETTINGS.P2P_relays,
        P2P_AppID: environment.p2p_app_id?.trim() || P2P_DEFAULT_SETTINGS.P2P_AppID,
        P2P_AutoStart: optionalBoolean(environment, "p2p_auto_start", P2P_DEFAULT_SETTINGS.P2P_AutoStart),
        P2P_AutoBroadcast: optionalBoolean(environment, "p2p_auto_broadcast", P2P_DEFAULT_SETTINGS.P2P_AutoBroadcast),
    });
    applyEncryptedVaultSettings(settings, environment);
...
function parseRemoteType(environment: SetupGeneratorEnvironment): SetupRemoteType {
...
export function createSetupSettings(environment: SetupGeneratorEnvironment): {
    remoteType: SetupRemoteType;
...
export async function generateSetupURI(environment: SetupGeneratorEnvironment): Promise<GeneratedSetupURI> {
    const setupPassphrase = environment.uri_passphrase?.trim() || generateSecret();
...
export async function runSetupURIGenerator(
    ...
    console.log("Your passphrase for the Setup URI is:", generated.setupPassphrase);
