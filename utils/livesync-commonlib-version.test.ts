...
Deno.test("standalone utilities select one exact Commonlib registry version", async () => {
    ...
    const selectedVersions = (await Promise.all(facadeURLs.map((url) => Deno.readTextFile(url)))).flatMap((source) =>
        [...source.matchAll(/npm:@vrtmrz\/livesync-commonlib@([^/"]+)\//gu)].map((match) => match[1])
    );
    ...
    assert(
        ...
        `Commonlib specifiers do not all select ${LIVESYNC_COMMONLIB_VERSION}: ${selectedVersions.join(", ")}`
    );
});
